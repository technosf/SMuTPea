 ...
 * A SMTP session.
 * <p>
 * Defines a SMTP session, handling {@code SessionState} management, calling an {@code MTA} implementation with events
 * and for responses.
 * 
...
public final class Session
{
	 ...
	 * <p>
	 * The {@code Session} will use the given {@code MTA} to process a {@code Command} and provide the response.
	 * 
	 * @param mta
	...
	/**
	 * Returns the {@code Buffer} for this {@code Session}
	 * 
	 * @return the {@code Buffer} for this {@code Session}
	 */
	 ...
	 * <p>
	 * Takes the client input provided by the {@code MTA} and processes it according to the {@code SessionState}. This
	 * method is synchronized to ensure that {@code SessionState} is updated atomically.
	 * 
	 * @param line
	...
	/**
	 * Process {@code MTA} input while the {@code Session} is in a {@literal COMMAND} state.
	 * <p>
	 * Expects and parses SMTP command input, passing the result to the {@code MTA} to decide the return code and
	 * description.
	 * 
	 * @param line
	...
	/**
	 * Processes {@code MTA} input while the {@code Session} is in the <em>DATA</em> state.
	 * <p>
	 * Expects and collates email body input until the end-of-data signifier (a containing a single period) is received.
	 * When the signifier is received the message is <em>sent</em> from the MTA and the session state moved back to
	 * <em>COMMAND</em>.
	 * 
	 * @param line
	 *            the input line from the {@code MTA}.
