 ...
 * <p>
 * A state machine defining valid transitions from a given state to a new state for a given command. Synthesized from
...
{
	private static final String CONST_MSG_ERR_SESSION_CLOSED = "Session is CLOSED.";
	private static final String CONST_FMT_INVALID_TX = "Invalid State transition [%1$s, %2$s].";
	...
	/**
	 * Identifies the the next {@code SessionState} when transitioning from a starting {@code SessionState} using a
	 * given {@code Command}.
	 * 
	 * @param startingState
	 ...
	 * @return the next {@code SessionState} from the current {@code SessionState} using the given {@code Command}
	 * @throws SessionClosedException
	 *             the {@code Session} has been closed and cannot transition
	 * @throws SessionStateException
	 *             cannot transition from the current {@code SessionState} using the given {@code Command}
	 */
					...
					throws SessionClosedException, SessionStateException
	{
		...
		{
			throw new SessionStateException(String.format(CONST_FMT_INVALID_TX, startingState,
							command));
		}
	...
	/**
	 * Helper method to generate a {@code Map.Entry} containing the given {@code SessionState} and {@code Command}
	 * 
	 * @param state
	 *            The {@code SessionState}
	 * @param command
	 *            The {@code Command}
	 * @return the {@code Map.Entry} containing the given {@code SessionState} and {@code Command}
	 */
