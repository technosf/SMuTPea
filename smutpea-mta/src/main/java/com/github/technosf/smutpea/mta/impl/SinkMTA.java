...
import com.github.technosf.smutpea.core.impl.rfc2821.ReplyCodes;
import com.github.technosf.smutpea.mta.AbstractMTA;

/**
 * {@code SinkMTA} is a basic MTA that dumps all email passed to it for
 * transfer.
 * <p>
 * Although {@code SinkMTA} is a valid MTA, no validation of destinations, hosts
 * or email addresses is done - everything
 * is accepted so that to the client, everything looks OK unless the client
 * violates the RFC.
 * <p>
 * {@code SinkMTA} uses {@code NullBuffer} and the mail <em>Buffer</em>, so no
 * data sent to the MTA is retrievable.
 * 
 * @author technosf
 * @since 0.0.1
 * @version 0.0.1
...
public final class SinkMTA
        extends AbstractMTA<Commands>
        implements MTA<Commands>
{
    ...
    */
    ...
    {
        super(CONST_MTA_NAME, domain, ReplyCodes._220, ReplyCodes._500);
    }


    public Class<Commands> getCommandClass()
    {
     ...
     * {@inheritDoc}
     * 
     * @see com.github.technosf.smutpea.core.impl.AbstractMTA#processValidCommand(com.github.technosf.smutpea.core.impl.rfc2821.Commands.CommandLine)
     */
    @Override
    protected void processValidCommand(CommandLine<Commands> commandLine)
            throws MTAException
    {
        ReplyCodes replyCode;

        switch (commandLine.getCommand()) {
            case DATA:
                replyCode = ReplyCodes._354;
                break;
            case HELP:
                replyCode = ReplyCodes._214;
                break;
            case QUIT:
                replyCode = ReplyCodes._221;
                break;
            case RSET:
            case VRFY:
            case EXPN:
     ...
     * {@inheritDoc}
     * 
     * @see com.github.technosf.smutpea.core.impl.AbstractMTA#processInvalidCommand(com.github.technosf.smutpea.core.impl.rfc2821.Commands.CommandLine)
     */
    @Override
    protected void processInvalidCommand(CommandLine<Commands> commandLine)
            throws MTAException
    {
        ReplyCodes replyCode = ReplyCodes._503;
    ...
    {
        setResponse(ReplyCodes._250,
                String.format("%1$s %2$s",
                        ReplyCodes._250.getCode(),
